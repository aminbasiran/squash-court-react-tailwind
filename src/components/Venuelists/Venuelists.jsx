...
const Venuelists = ({filter}) => {
    ...
    return (
        <>
            {state.isLoading ? <div className='mx-auto'><ClipLoader className="mt-10" color="#db70d6" size={50}/> </div> : <main className='flex flex-col space-y-9 '>
            ...
            {state.courts.filter(filtered => filtered.region.toLowerCase().includes(filter.toLowerCase())).length === 0 && <h1 className="text-2xl mt-10 text-zinc-400">No results found. Try another keyword</h1>}
            </main> }
        </>
    )
}
